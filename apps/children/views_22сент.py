        ...
        try:
            ...
            for index, row in df_limited.iterrows():
                ...
                try:
                    ...
                    # Преобразуем пол
                    gender = 'M'  # По умолчанию мужской
                    if pol:
                        pol_lower = pol.lower().strip()
                        if pol_lower in ['м', 'муж', 'мужской', 'male', 'm', 'муж.']:
                            gender = 'M'
                        elif pol_lower in ['ж', 'жен', 'женский', 'female', 'f', 'жен.']:
                            gender = 'F'

                    print(f"DEBUG: Преобразованный пол: {gender}")
                    ...
                    if direction_name and studio_name:
                        ...
                        # Ищем педагога через таблицу доступа (исключаем администраторов и методистов)
                        try:
                            print(f"DEBUG: Поиск педагогов с доступом к студии '{studio_name}'")

                            # Получаем текущего пользователя как педагога
                            try:
                                current_teacher = Teacher.objects.get(user=request.user)
                                print(f"DEBUG: Текущий пользователь - педагог: {current_teacher}")
                            except Teacher.DoesNotExist:
                                error_msg = f'Текущий пользователь {request.user.username} не является педагогом'
                                print(f"DEBUG: {error_msg}")
                                errors.append(f'Строка {index + 2}: {error_msg}')
                                error_details.append(f'Строка {index + 2}: {error_msg}')
                                continue

                            # Проверяем, есть ли у педагога доступ к этой студии
                            teacher_access = TeacherStudioAccess.objects.filter(
                                teacher=current_teacher,
                                studio=studio
                            ).first()

                            if not teacher_access:
                                error_msg = f'Педагог {current_teacher} не имеет доступа к студии "{studio_name}"'
                                print(f"DEBUG: {error_msg}")
                                errors.append(f'Строка {index + 2}: {error_msg}')
                                error_details.append(f'Строка {index + 2}: {error_msg}')
                                continue

                            # Используем текущего педагога
                            teacher = current_teacher
                            print(f"DEBUG: Педагог с доступом найден: {teacher}")
                        ...
                        if not enrollment_exists:
                            ...
                            # Получаем педагога (пока заглушка - текущий пользователь как педагог)
                            try:
                                teacher = Teacher.objects.get(user=request.user)
                                print(f"DEBUG: Найден педагог: {teacher}")
                            except Teacher.DoesNotExist:
                                # Создаем педагога для текущего пользователя, если его нет
                                ...
                                print(f"DEBUG: Создан педагог: {teacher_name}")
